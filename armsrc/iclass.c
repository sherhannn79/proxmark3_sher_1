...
static tUartIc Uart_ice;

static void uart_reset(void) {
    Uart_ice.frame_done = false;
    Uart_ice.synced = false;
    Uart_ice.frame = false;
}
        ...
        if (n_buf == 8) {
            ...
            switch (buf) {
                ...
                case 0xdf:    // eof - 1101 1111
                    Uart_ice.frame = false;
                    Uart_ice.synced = false;
                    Uart_ice.frame_done = true;
                    break;
                default:
                    Uart_ice.frame = false;
                    Uart_ice.synced = false;
                    Dbprintf("[-] bad %02X at %d:%d", buf, Uart_ice.len, nmsg_byte);
            }

            if (Uart_ice.frame) {   // data bits
                nmsg_byte += 2;
                if (nmsg_byte >= 8) {
                    Uart_ice.buf[Uart_ice.len++] = msg_byte;
                    nmsg_byte = 0;
                }
            }
            n_buf = 0;
...
again:
    ...
    //MOLSER
	//window += 3 - lz;
	window += 3 - (__builtin_clz(falling) - 24 + window);
...
static bool GetIClassCommandFromReader(uint8_t *received, int *len, int maxLen) {
    ...
    for (;;) {
        ...
        if (AT91C_BASE_SSC->SSC_SR & (AT91C_SSC_RXRDY)) {
            ...
            //uart_samples(b);
			uart_samples((uint8_t)AT91C_BASE_SSC->SSC_RHR);
            if (Uart_ice.frame_done) {
                *len = Uart_ice.len;
                return true;
            }
        }
...
static uint8_t opt_encode4Bits(uint8_t b) {
	switch (b & 0xF) {
							//  1       2         3         4
	  case 15: return 0x55; // 1111 -> 1111 -> 01010101 -> 0x55
	  case 14: return 0x95; // 1110 -> 0111 -> 10010101 -> 0x95
...
static void opt_CodeIClassTagAnswer(const uint8_t *cmd, int len) {	
	ToSendReset();
	ToSend[++ToSendMax] = 0x1D;
	for(int i = 0; i < len; i++) {
		ToSend[++ToSendMax] = opt_encode4Bits(cmd[i]); 		// least significant half
		ToSend[++ToSendMax] = opt_encode4Bits(cmd[i] >> 4);	// most significant half
	}	
	ToSend[++ToSendMax] = 0xB8;	
	ToSendMax++;
}
 ...
 */
int doIClassSimulation(int simulationMode, uint8_t *reader_mac_buf, int delay_for_check_res) {
	...
	//uint8_t is_one_page_tag = 1;
	bool cipher_changed = false;
	//bool do_log_trace = true;		
...
//  State cipher_state_reserve;
	...
	Dbprintf("Delay for check responce %d", delay_for_check_res);
	...
	//is_one_page_tag = ((conf_data[4] & 0x10) >> 4);
	//Dbprintf(" is_one_page_tag: %d", is_one_page_tag);
	...
	// Anticollision CSN
	opt_CodeIClassTagAnswer(anticoll_data, sizeof(anticoll_data));
	memcpy(resp_anticoll, ToSend, ToSendMax);
	resp_anticoll_len = ToSendMax;

	// CSN
	opt_CodeIClassTagAnswer(csn_data, sizeof(csn_data));
	memcpy(resp_csn, ToSend, ToSendMax);
	resp_csn_len = ToSendMax;

	// Configuration	
	opt_CodeIClassTagAnswer(conf_data, sizeof(conf_data));
	...
	// Application Issuer Area
	opt_CodeIClassTagAnswer(aia_data, sizeof(aia_data));
	...
	//This is used for responding to READ4-block commands or other data which is dynamically generated
	//First the 'trace'-data, not encoded for FPGA
	uint8_t *data4_generic_trace = BigBuf_malloc((8*4) + 2);//32 bytes data + 2byte CRC is max tag answer
	//Then storage for the modulated data
	//Each bit is doubled when modulated for FPGA, and we also have SOF and EOF (2 bytes)
	uint8_t *data4_response = BigBuf_malloc( ((8*4)+2) * 2 + 2);
	...
	while (!exitLoop) {
		...
		if ((receivedCmd[0] == ICLASS_CMD_CHECK) || (receivedCmd[0] == ICLASS_CMD_CHECK_KC)){ // 0x05 0x95
			...
			if (simulationMode == MODE_FULLSIM)  {
				...
				// 	check_count = 0;
					...
					trace_data_size = 4;
					opt_CodeIClassTagAnswer(trace_data, trace_data_size);
		...
		} else if (receivedCmd[0] == ICLASS_CMD_ACTALL) {
			...
			trace_data = NULL;
			trace_data_size = 0;
		} else if (receivedCmd[0] == ICLASS_CMD_READ_OR_IDENTIFY && len == 4) {
			if(simulationMode == MODE_FULLSIM){
				//check_count = 0;
				//Read block
			...
			trace_data_size = 34;
			opt_CodeIClassTagAnswer(trace_data, trace_data_size);
			memcpy(data4_response, ToSend, ToSendMax);
			modulated_response = data4_response;
			modulated_response_size = ToSendMax;
...
void SimulateIClass(uint32_t arg0, uint32_t arg1, uint32_t arg2, uint8_t *datain) {
	...
	uint16_t delay_for_check_res = arg2;
	FpgaDownloadAndGo(FPGA_BITSTREAM_HF);
	...
	if (simType == 0) {
		...
		doIClassSimulation(MODE_SIM_CSN,NULL,delay_for_check_res);
	} else if (simType == 1) {
		//Default CSN
		...
		for ( ; i < numberOfCSNS && i*8+8 < USB_CMD_DATA_SIZE; i++) {
			...
			if (doIClassSimulation(MODE_EXIT_AFTER_MAC,mac_responses+i*8,delay_for_check_res)) {
		...
		doIClassSimulation(MODE_FULLSIM, NULL, delay_for_check_res);
	} else {
 ...
 *         2 = Got CSN and CC
 */
static uint8_t handshakeIclassTag_ext(uint8_t *card_data, bool use_credit_key, uint8_t pageNbr) {
	//Dbprintf("handshakeIclassTag_ext-pageNbr: %d", pageNbr);
	static uint8_t act_all[]     = { 0x0a };
	//static uint8_t identify[]    = { 0x0c };
	static uint8_t identify[]    = { 0x0c, 0x00, 0x73, 0x33 };
	static uint8_t select[]      = { 0x81, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
	static uint8_t readcheck_cc[]= { 0x88, 0x02 };
	static uint8_t selectPage[]   = { ICLASS_CMD_PAGESEL, 0x00, 0x00, 0x00 };
	if (use_credit_key)
		readcheck_cc[0] = 0x18;
	else
		readcheck_cc[0] = 0x88;
	...
	//Select Page
	if(pageNbr != 255 ){
		selectPage[1] = pageNbr;
		AppendCrc(selectPage+1, 1);
		ReaderTransmitIClass(selectPage, sizeof(selectPage));

		//We expect a 10-byte response here, 8 byte block-config and 2 byte CRC
		uint8_t len = ReaderReceiveIClass(resp);
		if (len != 10) return read_status;//Fail
	}
...
static uint8_t handshakeIclassTag(uint8_t *card_data) {
	return handshakeIclassTag_ext(card_data, false,255);
}


// Reader iClass Anticollission
void ReaderIClass(uint8_t arg0, uint8_t pageNbr) {

	Dbprintf("ReaderIClass-pageNbr: %d", pageNbr);
	...
	//MOLSER
	//uint8_t pageSel[] = { ICLASS_CMD_PAGESEL, 0x00,  0x73,  0x33 };
	...
	while (!userCancelled) {
		...
		read_status = handshakeIclassTag_ext(card_data, use_credit_key,pageNbr);
		...
		// if (sendCmdGetResponseWithRetries(pageSel, sizeof(readConf), resp, 10, 10)) {

		// } else {
		// 	Dbprintf("Failed to get data for PageSel");
		// }
